...
export default function AddAttibute({
  attributes,
  isSaveSettings,
  setAttributes,
}: {
  ...
  return (
    <div>
      <div className="justify-content-end flex">
        ...
        <Button
          label="Add Attribute"
          onClick={() => append({ attribute: '', attributeValues: [] })}
          outlined
        />
      </div>
      <div>
        {fields.map((item, index) => (
          <div key={item.id} className="align-items-center mt-4 flex">
            <div className="mr-2 w-5">
              <Controller
                ...
                render={({ field }) => (
                  <Select
                    options={
                      data?.getAllProductAttribute?.length
                        ? data.getAllProductAttribute.map((d: any) => ({
                            label: d.name,
                            value: d._id,
                          }))
                        : []
                    ...
                    onChange={(val: any) => {
                      ...
                      setAttributeValues(
                        data?.getAllProductAttribute
                          ?.find((el: any) => el._id === val.value)
                          ?.values?.map(
                            (d: any) =>
                              ({ label: d?.valueName, value: d?._id }) || [],
                          ),
                      );
                    }}
