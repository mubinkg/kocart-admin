...
export default function Page() {
  const { data: sliderList, refetch } = useQuery(GET_SLIDER_LIST, {
    variables: { limit: 1000, offset: 0 },
    fetchPolicy: 'no-cache',
  });
  const [createSlider, { loading: sliderCreateLoading }] =
    useMutation(CREATE_SLIDER);
  ...
  const loadCategoryOptions: any = async (val: string, callback: any) => {
    ...
    callback(
      data?.data?.sliderCategory?.map((d: any) => ({
        label: d?.name,
        value: d?._id,
      })) || [],
    );
  ...
  const brandImageRenderer = (item: any) => (
    <Image src={item?.image} alt="Brand Image" width="200" />
  );

  return (
    <div>
      <h1 className="ml-4 font-light">
        Slider Image For Add-on Offers and other benefits
      </h1>
      <Card className="m-4 p-2">
        ...
        <DataTable
          lazy
          totalRecords={
            sliderList?.adminSliderList?.count
              ? sliderList?.adminSliderList?.count
              : 0
          }
          onPage={value => console.log(value)}
          value={
            sliderList?.adminSliderList?.sliders
              ? sliderList?.adminSliderList?.sliders
              : []
          }
          paginator
        ...
        <Card>
          <div className="flex-column flex">
            <p className="my-3 font-semibold">
              Slider Type <span style={{ color: 'red' }}>*</span>
            </p>
            <Select
              options={
                slitedTypeList?.sliderType?.length
                  ? slitedTypeList?.sliderType?.map((d: any) => ({
                      label: d.type,
                      value: d._id,
                      type_id: d.type_id,
                    }))
          ...
          <div className="flex-column flex">
            <p className="my-3 font-semibold">
              Slider Image <span style={{ color: 'red' }}>*</span>
            </p>
            <FileUpload className="w-full" ref={sliderImageRef} />
          </div>
          <Button
            label={sliderCreateLoading ? 'Loading...' : 'Submit'}
            className="mt-3"
