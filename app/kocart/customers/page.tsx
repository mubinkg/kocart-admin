...
export default function Page() {
  const { data: customerData, refetch } = useQuery(ADMIN_CUSTOMER_LIST, {
    variables: { limit: 1000, offset: 0, query: '' },
  });

  const items = [{ label: 'Customers' }, { label: 'Customers List' }];
    ...
    <div>
      ...
      <Card className="m-4 p-2">
        <DataTable
          lazy
          totalRecords={
            customerData?.customerAdminList?.count
              ? customerData?.customerAdminList?.count
              : 0
          }
          onPage={value => console.log(value)}
          value={
            customerData?.customerAdminList?.customers
              ? customerData?.customerAdminList?.customers
              : []
          }
          paginator
