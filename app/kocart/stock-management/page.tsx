...
export default function Brand() {
  ...
  const [updateStock, { loading: updateStockLoading }] =
  ...
  const items = [{ label: 'Home' }, { label: 'Product Stock' }];

  const brandImageRenderer = (item: any) => (
    <Image src={item?.product?.pro_input_image} alt="Brand Image" width="200" />
  );
  ...
  const attributeActionRenderer = (item: any) => (
    <i
      onClick={() => {
        setVisible(true);
        setItem(item);
  ...
  return (
    <div>
      ...
      <Card className="m-4">
        <DataTable
          totalRecords={
            data?.getAdminStockList?.count ? data?.getAdminStockList.count : 0
          }
          lazy
          ...
          loading={loading}
          value={
            data?.getAdminStockList?.stock ? data?.getAdminStockList?.stock : []
          }
      ...
      >
        ...
        <InputText
          ...
          disabled
        />
        <div className="flex-column flex w-12">
          ...
          <Select
            ...
            defaultValue={stockUpdateOptions[0]}
          />
        </div>
        <Button onClick={submitHandler} className="mt-4 w-12" label="Submit" />
      </Dialog>
    </div>
  );
}
