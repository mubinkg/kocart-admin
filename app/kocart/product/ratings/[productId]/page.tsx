...
export default function Page({ params }: any) {
  const [getFaqList, { data: faqList, loading }] = useLazyQuery(
    ADMIN_PRODUCT_FAQ_LIST,
  ...
  const items = [{ label: 'Product' }, { label: 'Product Rating List' }];
    ...
    <div>
      ...
      <Card className="m-4 p-2">
        <DataTable
          ...
          loading={loading}
          totalRecords={
            faqList?.getProductFaqList?.count
              ? faqList?.getProductFaqList?.count
              : 0
          }
          onPage={value => setPageData(value)}
          value={
            faqList?.getProductFaqList?.faqs
              ? faqList?.getProductFaqList?.faqs
              : []
          }
          paginator
