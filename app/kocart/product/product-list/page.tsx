...
export default function ProductList() {
  ...
  const productImageRenderer = (url: any) => (
    <Image alt="Category Image" width="100" src={url} />
  );

  const items = [{ label: 'Product' }, { label: 'Product List' }];
  ...
  const productAction = (item: any) => (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      ></i>
      <i
        className="pi pi-pen-to-square mt-3 cursor-pointer"
        onClick={() => navigation.push(`/kocart/product/edit/${item._id}`)}
      ></i>
      <i
        className="pi pi-trash mt-3 cursor-pointer"
        onClick={() =>
          Swal.fire({
      ...
      ></i>
      <i
        className="pi pi-star mt-3 cursor-pointer"
        onClick={() => navigation.push(`/kocart/product/ratings/${item._id}`)}
      ></i>
      <i
        className="pi pi-question-circle mt-3 cursor-pointer"
  ...
  return (
    <div>
      ...
      <Card className="m-4">
        ...
        >
          <Column
            body={(item: any) => productImageRenderer(item?.pro_input_image)}
            header="Image"
          />
          ...
          <Column
            body={() => <Rating value={0} cancel={false} />}
            header="Rating"
          />
          <Column body={(item: any) => productAction(item)} header="Action" />
        </DataTable>
        <Dialog
          className="sm:w-12 lg:w-8"
              ...
              <div className="w-6">
                ...
                {product?.variant === 'variable' ? (
                  <div>
                    ...
                    <DataTable value={product?.productvariants || []}>
