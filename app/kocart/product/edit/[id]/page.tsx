...
import { useProductDetails } from '@/hooks/product/useGetProductDetails';
...
export default function AddProduct({ params: { id } }: IProps) {
  ...
  const { register, setValue, watch, handleSubmit, control, getValues } =
  ...
  useEffect(() => {
    ...
    if (productDetails) {
      ...
      setValue(
        ...
        productDetails.extra_input_description,
      );
      ...
      setValue(
        ...
        productDetails?.total_allowed_quantity,
      );
  ...
  return (
    <div>
      <BreadCrumb className="m-4" model={updateProduct} />
      <Card className="m-4">
          ...
          <div className="flex-column flex w-6">
            ...
            <Select
              ...
              value={productType.find(
                val => val.value === getValues('product_type'),
              )}
              isClearable
              isSearchable
              isLoading
          ...
          <div className="justfy-content-between flex flex-wrap">
              ...
              <div className="flex-column flex w-3 pr-3">
                ...
                <Select
                  ...
                  value={indicatorOptions.find(
                    val => val.value === indicatorMap[watch('indicator') || 0],
                  )}
                />
              </div>
            ) : (
            ...
            <div className="flex-column flex w-3 pr-3">
              ...
              <Select
                ...
                value={
                  brandList?.adminBrandList?.brands?.length
                    ? brandList?.adminBrandList?.brands
                        .map((d: any) => ({ label: d.name, value: d._id }))
                        .find((val: any) => val.value === watch('brand'))
                    : {}
                }
              />
            </div>
