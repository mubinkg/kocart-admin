...
export default function Seller() {
  const [getSellers, { data, loading, refetch }] = useLazyQuery(GET_SELLET, {
  ...
  const actionBody = (data: any) => (
    <Button
      onClick={() =>
        updateStatus(data._id, data.status === 'active' ? 'INACTIVE' : 'ACTIVE')
      }
      label={data.status === 'active' ? 'Inactive' : 'Active'}
      style={{ background: data.status === 'active' ? 'red' : '' }}
    />
  );

  const items = [{ label: 'Seller' }, { label: 'Seller List' }];
