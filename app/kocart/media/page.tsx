...
const items = [{ label: 'Media' }, { label: 'Media List' }];
...
export default function Page() {
  ...
  const brandImageRenderer = (item: any) => (
    <Image src={item?.file} alt="Brand Image" width="200" />
  );
  const mediaSizeRenderer = (item: any) => <p>{item.size / 1000} kb</p>;
  const mediaNameRenderer = (item: any) => (
    <p style={{ maxWidth: '150px', wordWrap: 'break-word' }}>{item.name}</p>
  );
  ...
  return (
    <div>
      ...
      <Card className="m-4">
        ...
        <Button
          onClick={createMediaHandler}
          label={loading ? 'Loading...' : 'Upload'}
          className="mt-4"
        />
        <div className="justify-content-end my-4 flex gap-2">
          <InputText
            style={{ height: '40px' }}
            value={query}
        ...
        <DataTable
          totalRecords={
            mediaList?.adminMedia?.count ? mediaList?.adminMedia?.count : 0
          }
          value={
            mediaList?.adminMedia?.media ? mediaList?.adminMedia?.media : []
          }
          lazy
          paginator
          rows={1000}
