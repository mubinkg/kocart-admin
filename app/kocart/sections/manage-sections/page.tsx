...
import {
  ...
  ProductTypes,
  SectionStyle,
} from '@/data/featured-section/types';
...
import {
  ...
  GET_ADMIN_SECTIONS,
  GET_ALL_CATEGORY,
  GET_PRODUCT_FOR_FEATURED_SECTION,
} from '@/graphql/fetured-section';
import {
  productTypeOptions,
  styleOptions,
} from '@/graphql/fetured-section/data';
...
export default function Page() {
  ...
  const { data: categoryList } = useQuery(GET_ALL_CATEGORY, {
    fetchPolicy: 'no-cache',
  });
  const [getProduct] = useLazyQuery(GET_PRODUCT_FOR_FEATURED_SECTION);
  const [createFeaturedSection, { loading: createFeaturedSectionLoading }] =
    useMutation(CREATE_FEATURED_SECTION);

  const [getSections, { data, loading }] = useLazyQuery(GET_ADMIN_SECTIONS, {
    fetchPolicy: 'no-cache',
  });
  ...
  const loadProductOptions: any = async (val: string, callback: any) => {
    if (watch('productType')) {
      ...
      callback(
        products.map((d: any) => ({ label: d.pro_input_name, value: d._id })),
      );
    }
  };
  ...
  const items = [{ label: 'Featured Sections' }, { label: 'Manage Sections' }];

  const sectionCategoryRenderer = (item: any) => (
    <p>{item.categories.join(' , ')}</p>
  );
  const sectionProductRenderer = (item: any) => (
    <p style={{ maxWidth: '300px', wordWrap: 'break-word' }}>
      {item?.products?.map((d: any) => d._id)?.join(',') || ''}
    </p>
  );
    ...
    <div>
      ...
      <Card className="m-4" title="Featured Sections">
        ...
        <DataTable
          ...
          totalRecords={
            data?.getAdminSetions?.count ? data?.getAdminSetions?.count : 0
          }
          onPage={values => setPageData(values)}
          value={
            data?.getAdminSetions?.sections
              ? data?.getAdminSetions?.sections
              : []
          }
          paginator
      ...
      <Dialog
        header="Create New Fatured Section"
        visible={visible}
        style={{ width: '50vw' }}
      ...
      >
        <div className="flex-column flex">
          <p className="mb-2 font-semibold">
            Title for section <span style={{ color: 'red' }}>*</span>
          </p>
          <InputText
            {...register('title')}
          ...
          <Controller
            ...
            render={({ field }) => (
              <Select
                {...field}
                options={
                  categoryList?.getAllCategory?.map((d: any) => ({
                    label: d.name,
                    value: d._id,
                  })) || []
                }
        ...
        <div className="flex-column flex">
          <p className="mb-2 font-semibold">
            Product Types <span style={{ color: 'red' }}>*</span>
          </p>
          <Controller
            name="productType"
            control={control}
        ...
        watch('productType')?.value === 'DIGITAL_PRODUCT' ? (
          <div className="flex-column flex">
            <p className="mb-2 font-semibold">
        ...
        <Button
          label={createFeaturedSectionLoading ? 'Loading...' : 'Submit'}
          className="my-3"
          onClick={handleSubmit(submitHandler)}
          disabled={createFeaturedSectionLoading}
        />
      </Dialog>
    </div>
  );
