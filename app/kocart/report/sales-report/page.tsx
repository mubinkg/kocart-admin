...
export default function Page() {
  const { data, loading, error, refetch } = useQuery(SALES_REPORT, {
  ...
  const items = [{ label: 'Report' }, { label: 'Sales Report' }];

  const productNameRenderer = (item: any) => (
    <p>
      {item?.product_variants
        ?.map((data: any) => data?.product?.pro_input_name)
        .join(',') || ''}
    </p>
  );
  ...
  return (
    <div>
      ...
      <Card className="m-4">
        <DataTable
          lazy
          totalRecords={data?.salesReport?.count ? data?.salesReport?.count : 0}
          onPage={value => console.log(value)}
          value={data?.salesReport?.orders ? data?.salesReport?.orders : []}
          paginator
          rows={5}
